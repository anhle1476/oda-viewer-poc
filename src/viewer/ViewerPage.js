...
class Viewer extends Component {
	...
	connectViewpointListComponentEvents() {
		...
		this.subscribeSubjectContext.subscribe(
			this.viewpointListEvents.onClose,
			() => this.toggle("isVisibleViewPointView")()
		);
		this.subscribeSubjectContext.subscribe(
			this.viewpointListEvents.onRemoveViewpoint,
			async (vp) => {
				try {
					await this.state.activeModel.deleteViewpoint(vp.guid);
					this.context.app.addNotification(
						"success",
		...
		this.subscribeSubjectContext.subscribe(
			this.viewpointListEvents.onSelectViewpoint,
			async (vp) => {
				...
				if (
					...
					const model = this.state.modelList.find(
						(model) => model.id === viewpointModelId
					);
					if (model) {
						try {
	...
	connectHeaderComponentEvents() {
		...
		this.subscribeSubjectContext.subscribe(
			this.headerEventsConnection.onOpenModel,
			(model) => this.openModel(model)
		);

		this.subscribeSubjectContext.subscribe(
			this.headerEventsConnection.onOpenObjectExplorer,
			() => this.toggle("isVisibleObjectExplorer")()
		);

		this.subscribeSubjectContext.subscribe(
			this.headerEventsConnection.onOpenProperties,
			() => {
				this.toggle("isVisibleProperties")();
				if (this.lastProperties) {
					setTimeout(() => {
						this.propertiesEvents.completeLoadProperties.next(
							this.lastProperties
						);
					}, 0);
				}
		...
		this.subscribeSubjectContext.subscribe(
			this.headerEventsConnection.onOpenViewpointView,
			async () => {
		...
		this.subscribeSubjectContext.subscribe(
			this.headerEventsConnection.onOpenEdit,
			() => {
	...
	connectDraggerComponentEvents() {
		...
		this.subscribeSubjectContext.subscribe(
			this.draggerEventsConnection.onActivateDraggerByName,
			(draggerName) => {
		...
		this.subscribeSubjectContext.subscribe(
			this.draggerEventsConnection.onClearViewPoint,
			() => {
		...
		this.subscribeSubjectContext.subscribe(
			this.draggerEventsConnection.onSaveViewPoint,
			async () => {
				const viewPointPlugin = new ViewPointHelper(this.viewer.visLib());
				...
				this.state.activeModel &&
					(vp.custom_fields.modelId = this.state.activeModel.id);
				...
				if (this.state.isVisibleViewPointView && newVp) {
					if (!newVp.snapshot) {
						newVp.snapshot = await this.state.activeModel.getSnapshot(
							newVp.guid
						);
					}
	...
	connectViewActionComponentEvents() {
		...
		this.subscribeSubjectContext.subscribe(
			this.viewActionEvents.onChangeVisualStyle,
			(name) => {
				execWithLoadIndicator(() =>
					this.viewer.visViewer().setActiveVisualStyle(name)
				);
			}
		);

		this.subscribeSubjectContext.subscribe(
			this.viewActionEvents.onChangeViewType,
			(type) => {
				const defViewPos = this.viewer.visLib().DefaultViewPosition;
				this.viewer
					.visViewer()
					.setDefaultViewPositionWithAnimation(defViewPos[type]);
			}
		);

		this.subscribeSubjectContext.subscribe(
			this.viewActionEvents.onChangeViewToDefaultValue,
			() => this.viewer.visViewer().zoomExtents()
		);

		const onUnselect = () =>
			execWithLoadIndicator(() => this.viewer.visViewer().unselect());
		const onIsolateSelectedObjects = () =>
			execWithLoadIndicator(() =>
				this.viewer.visViewer().isolateSelectedObjects(false)
			);
		const onHideSelectedObjects = () =>
			execWithLoadIndicator(() =>
		...
		this.subscribeSubjectContext.subscribe(
			this.viewActionEvents.onUnselect,
			onUnselect
		);
		this.subscribeSubjectContext.subscribe(
			this.viewActionEvents.onIsolateSelectedObjects,
			onIsolateSelectedObjects
		);
		this.subscribeSubjectContext.subscribe(
			this.viewActionEvents.onHideSelectedObjects,
			onHideSelectedObjects
		);
		this.subscribeSubjectContext.subscribe(
			this.viewActionEvents.onShowAll,
			onShowAll
		);
		this.subscribeSubjectContext.subscribe(
			this.viewActionEvents.onExplode,
			onExplode
		);
		this.subscribeSubjectContext.subscribe(
			this.viewActionEvents.onCollect,
			onCollect
		);
		this.subscribeSubjectContext.subscribe(
			this.viewActionEvents.onRegeneration,
			onRegeneration
		);
		this.subscribeSubjectContext.subscribe(
			this.viewActionEvents.onCreatePreview,
			onCreatePreview
		);

		this.subscribeSubjectContext.subscribe(
			this.contextMenuEvents.onUnselect,
			onUnselect
		);
		this.subscribeSubjectContext.subscribe(
			this.contextMenuEvents.onIsolateSelectedObjects,
			onIsolateSelectedObjects
		);
		this.subscribeSubjectContext.subscribe(
			this.contextMenuEvents.onHideSelectedObjects,
			onHideSelectedObjects
		);
		this.subscribeSubjectContext.subscribe(
			this.contextMenuEvents.onShowAll,
			onShowAll
		);
		this.subscribeSubjectContext.subscribe(
			this.contextMenuEvents.onExplode,
			onExplode
		);
		this.subscribeSubjectContext.subscribe(
			this.contextMenuEvents.onCollect,
			onCollect
		);
		this.subscribeSubjectContext.subscribe(
			this.contextMenuEvents.onRegeneration,
			onRegeneration
		);
		this.subscribeSubjectContext.subscribe(
			this.contextMenuEvents.onCreatePreview,
			onCreatePreview
		);
	...
	resetVisualStyleInformation = () => {
		this.viewActionEvents.changeDefaultVisualStyle.next(
			this.viewer.visViewer().getActiveVisualStyle()
		);
	};
	...
	async componentDidMount() {
		...
		this.headerEventsConnection.changeEnableProperties.next(
			this.modelContext.getHaveProperties()
		);
		...
		this.viewer
			.visViewer()
			.setExperimentalFunctionalityFlag("gpu_select", false);
	...
	openModel = async (model) => {
		try {
			...
			this.draggerEventsConnection.changeSupportViewPoint.next(
				this.modelContext.isSupportViewPoint()
			);
			this.headerEventsConnection.changeSupportViewPoint.next(
				this.modelContext.isSupportViewPoint()
			);
			this.headerEventsConnection.changeSupportTransform.next(
				this.modelContext.isSupportTransform()
			);
...

			{
				...
				this.headerEventsConnection.changeEnableObjectExplorer.next(
					!itr.done()
				);
				this.setState({
					geometryLoaded: true,
			...
			if (extFile === ".obj" || extFile === ".ifc")
				this.viewer.visViewer().setBackgroundColor([255, 255, 255]);
		...
		return (
			<div className="h-100 d-flex flex-column">
				...
				<div className="flex-grow-1 overflow-hidden">
					<canvas
						id="canvas"
						className={classNames(this.state.canvasClassName, "w-100 h-100")}
					/>

					{this.state.visualizeProgress && (
						<VisualizeProgress value={this.state.visualizeProgress} />
					)}
				...
				{this.state.isVisibleProperties ? (
					<PropertiesComponent eventsConnection={this.propertiesEvents} />
				) : null}
