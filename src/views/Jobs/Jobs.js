...
import { JOB_STATUS, StatusTag, statusMap } from "../../components";
...
class Jobs extends React.Component {
      ...
      render: (name, job) => {
        ...
        return (
          <Tooltip title={job.fileId ? "File" : "Assembly"} placement="right">
            {job.status === JOB_STATUS.DONE && name ? <Link to={to}>{name}</Link> : name || "(Deleted)"}
          </Tooltip>
        );
      },
      ellipsis: { showTitle: false },
  ...
  componentDidMount() {
    ...
    const isNeedUpdate = () => {
      return this.state.jobs.some((job) => job.name && !job.done);
    };
  ...
  getJobs = async (page, pageSize, status) => {
    ...
    this.setState({ loading: true, error: "", filters: { status: status } });
    try {
      const jobs = await JobsService.getJobs(page, pageSize, status);
      this.setState({
        jobs: jobs.result,
        loading: false,
        pagination: { ...pagination, total: jobs.allSize, current: page, pageSize: pageSize },
      });
    } catch (e) {
  ...
  handleTableChange = (pagination, filters) => {
    this.getJobs(pagination.current, pagination.pageSize, filters.status || []);
  };
