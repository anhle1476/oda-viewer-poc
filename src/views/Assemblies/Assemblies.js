...
import { StatusTag, JOB_STATUS, PreviewIcon, NameFilter } from "../../components";
...
class Assemblies extends React.Component {
  columns = [
    ...
    },
    {
      ...
      render: (name, assembly) =>
        assembly.status === JOB_STATUS.DONE ? <Link to={`/assemblies/${assembly.id}`}>{name}</Link> : name,
    ...
    },
    {
      ...
      render: (_, assembly) => {
        return (
          <div>
            {assembly.status === JOB_STATUS.DONE ? (
  ...
  componentDidMount() {
    ...
    const isNeedUpdate = () => {
      return this.state.assemblies.some((assembly) => !assembly.done).length;
    };
  ...
  getAssemblies = async (page, pageSize, name) => {
    ...
    this.setState({ loading: true, error: "", filters: { name: name } });
    try {
      const assemblies = await AssembliesService.getAssemblies(page, pageSize, name[0]);
      this.setState({
        assemblies: assemblies.result,
        loading: false,
        pagination: { ...pagination, total: assemblies.allSize, current: page, pageSize: pageSize },
      });
    } catch (e) {
  ...
  handleTableChange = (pagination, filters) => {
    this.getAssemblies(pagination.current, pagination.pageSize, filters.name || []);
  };
