...
window["getVisualizeLibInst"] = function getVisualizeLibInst(params) {
	function downloadWasm(url, progress) {
		...
		// TODO: remove, hack to solve the MIME type problem when host it in public folder
		url = "https://opencloud.azureedge.net/libs/visualizejs/23.7/Visualize.js.wasm";
	...
	Module["postRun"] = [];
	Module["TOTAL_MEMORY"] =
		params.TOTAL_MEMORY > 16777216 ? params.TOTAL_MEMORY : 16777216;
	...
	if (!isDataURI(wasmBinaryFile)) {
		...
		// TODO: remove, hack to explore the library
		wasmBinaryFile = "https://opencloud.azureedge.net/libs/visualizejs/23.7/Visualize.js.wasm";
	}
	function getBinary() {
		try {
			if (wasmBinary) {
	...
	var ASM_CONSTS = {
		...
		28081: function () {
			var _initializeBuffers = function (
				glCtx,
				frameBuffer,
				renderbuffer,
				frameTexture
			...
			var _unitializeBuffer = function (
				glCtx,
				frameBuffer,
				renderbuffer,
				frameTexture
			...
			Module.Viewer.prototype.addEmbedetFile = function (name, b) {
				console.log(
					"Method addEmbedetFile is deprecated use addEmbeddedFile, remove in next releas"
				);
			...
			Module.OdTvRasterImage.prototype.setBinaryData = function (
				format,
				width,
				height,
				iAlignment,
		...
		53748: function ($0) {
			...
			return Module["canvas"]
				? Module.canvas["width"] || canvas.clientWidth || $0
				: $0;
		},
		53973: function ($0) {
			if (!Module["canvas"]) {
			...
			return Module["canvas"]
				? Module.canvas["height"] || canvas.clientHeight || $0
				: $0;
		},
		358834: function ($0, $1) {
			Module.HpTrc.Usd += $1;
	...
	function readAsmConstArgs(sigPtr, buf) {
		...
		while ((ch = HEAPU8[sigPtr++ >>> 0])) {
			...
			readAsmConstArgsArray.push(
				double ? HEAPF64[buf++ >>> 1] : HEAP32[buf >>> 0]
			);
			++buf;
		}
	...
	var dynCall_fiiij = (Module["dynCall_fiiij"] = function () {
		return (dynCall_fiiij = Module["dynCall_fiiij"] =
			Module["asm"]["dynCall_fiiij"]).apply(null, arguments);
	});
	...
	var dynCall_vijjiiii = (Module["dynCall_vijjiiii"] = function () {
		return (dynCall_vijjiiii = Module["dynCall_vijjiiii"] =
	...
	var dynCall_jijj = (Module["dynCall_jijj"] = function () {
		return (dynCall_jijj = Module["dynCall_jijj"] =
			Module["asm"]["dynCall_jijj"]).apply(null, arguments);
	});
	var dynCall_viijjiii = (Module["dynCall_viijjiii"] = function () {
		return (dynCall_viijjiii = Module["dynCall_viijjiii"] =
	...
	if (!Module["addOnPostRun"] && Module.postRun) {
		Module["addOnPostRun"] = function (cb) {
			console.warn(
				"addOnPostRun(cb) is deprecated and will be removed in future, please use .postRun.push(cb)"
			);
